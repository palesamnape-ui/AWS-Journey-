...
No shortcuts, no copy-paste solutions—just me, two EC2 instances with mysterious connectivity issues, and networking fundamentals to apply.
...
First thing I did was fire up the AWS lab environment and navigated to the EC2 console. I had two instances waiting for me—Instance A and Instance B—both supposedly configured identically. Time to investigate.
...
Bingo. The root cause was immediately clear—Instance A had no public IP address, which meant it couldn't be accessed from the internet or reach out to the internet directly.
...
- **Instance A:** SSH connection failed—the public IP field was empty, so there was nothing to connect to
...
To provide a thorough analysis, I applied the OSI model troubleshooting approach—starting from the bottom layer (Physical) and working up. In AWS terms:

- **Physical Layer:** Region and Availability Zone (✓ working)
- **Network Layer:** Route tables, Internet Gateway, Subnets (✓ working)
- **Transport Layer:** Security groups, NACLs (✓ working)
- **Session Layer:** EC2 instance configuration (✗ **PROBLEM HERE**)

The issue wasn't with the AWS architecture—it was with the EC2 instance configuration itself. Instance A simply hadn't been assigned a public IP address at launch.
...
Instance A cannot reach the internet because it was not assigned a public IP address when launched. Private IP addresses are non-routable on the public internet by design—they're meant for internal communication within the VPC.
